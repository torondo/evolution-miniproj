...
names <- readRDS("gj_wd/Names_1920to2015.rds")
...
This step is to set up functions that will run the neutral model simulations. This is independent to the actual number of simulations run, allowing for flexibility in the model.
...
Run the specified number of simulations and collect observed extinctions. By changing `n_sims`, you can modify the simulations to your desired number of runs. 
For this notebook simulation, N = 10. Full model (N = 1000 simulations) will be included as an RDS for loading later. 
``` {r}
n_sims <- 10
sim_stats <- purrr::map(
  seq_len(n_sims),
...
## Statistical tests
#### Test 1: Overall test
Are overall extinction rates different from Wright-Fisher model expectations?
```{r test}
...
#### Test 3: Correlation
Do names that the Wright-Fisher neutral model predict will go extinct actually go extinct?
```{r correlation}
# Test on each name
...
```{r, echo=FALSE}
extinction_results$freq_bin <- ntile(extinction_results$f0, 10)

binned <- extinction_results %>%
  group_by(freq_bin) %>%
  summarise(obs  = (mean(observed)*100), exp  = (mean(expected)*100),
            .groups = "drop") %>%
  tidyr::pivot_longer(cols = c(obs, exp), names_to = "type", values_to = "extinction_rate")

plot1 <- ggplot(binned, aes(x = freq_bin, y = extinction_rate, colour = type)) +
  geom_line() +
  scale_colour_manual(values = c("obs" = "steelblue", "exp" = "red"), 
                      labels = c("obs" = "Observed", "exp" = "Neutral"), name = NULL) +
  labs(x = "Starting frequency bin", y = "Mean extinction rate (%)",   tag = "a)") +
  theme_classic(base_size = 18) +
  theme(axis.title = element_text(size = 18), axis.text = element_text(size = 16),
        legend.position = "inside", legend.position.inside = c(1.3, 0.2))
plot1
```

#### Plot 2: Rate of extinction per year (observed vs. expected)
```{r, echo=FALSE}
# observed: calculate number of names going extinct per year
plot2 <- ggplot(extinction, aes(x = endyear)) +
  geom_line(aes(y = obs_rate, color = "Observed")) +
  geom_line(aes(y = exp_rate, color = "Neutral")) +
  scale_colour_manual(values = c("Observed" = "steelblue","Neutral" = "red"), name = NULL) +
  labs(x = "Year", y = "Mean extinction rate (%)", tag = "b)") +
  theme_classic(base_size = 18) +
  theme(axis.title = element_text(size = 18), axis.text = element_text(size = 16),
        legend.position = "outside", legend.position.inside = c(0.2, 0.2))
plot2
```
